...
/*Mesh * Mesh::generaPrisma(GLdouble r, GLdouble h, int n)
{
	Mesh * m = new Mesh();
	m->primitive = GL_TRIANGLE_STRIP;
	m->numVertices = n*2 + 2;
	...
	for (i; i < n*2; i = i+2) {
		...
		x = 0.0 + r * cos(radians(ang));
		z = 0.0 + r * sin(radians(ang));

		m->vertices[i] = dvec3(x, h, z);
		m->vertices[i+1] = dvec3(x, 0, z);
		ang += incrAng;
	}

	m->vertices[i] = m->vertices[0];
	m->vertices[i+1] = m->vertices[1];
...
Mesh * Mesh::generaPiramide(GLdouble r, GLdouble h, int n)
{
	Mesh * m = new Mesh();
	m->primitive = GL_TRIANGLE_FAN;
	m->numVertices = n + 2;
	...
	for (int i = 1; i < m->numVertices; i++) {
		...
		x = 0.0 + r * cos(radians(angIni + incrAng * (i - 1)));
		z = 0.0 + r * sin(radians(angIni + incrAng * (i - 1)));

		m->vertices[i] = dvec3(x, 0, z);
	}
